...
class CircleChanger(object):
    ...
    def swell_or_shrink_repeatedly(self,
        ...
        for _ in range(times_to_swell_or_shrink):
            ...
            self.draw()
            j = amount_to_swell_or_shrink * -1
            self.swell_or_shrink_once(j)
            self.draw()
    ...
    def swallow(self, other_circle_changer):
        ...
        r = self.get_distance_from(other_circle_changer.circle.center) / 2
        color = self.colors + other_circle_changer.colors
        new = CircleChanger(x, y, r, 'black', color)
        return new
